...
from ..etdrk import ETDRKIntegrator
...
class CoreGenerator(ABC):
    ...
    @abstractmethod
    def __call__(self, f_mesh:FourierMesh,n_channel:int) -> Union[LinearCoef,NonlinearFunc]:
        pass
...
def check_value_with_mesh(u:torch.Tensor,
    ...
    if len(value_shape)-2 != n_dim:
        raise ValueError(f"the value shape {value_shape} is not compatible with mesh dim {n_dim}")
    for i in range(n_dim):
        if value_shape[i+2] != mesh[i][2]:
            raise ValueError(f"Expect to have {mesh[i][2]} points in dim {i} but got {value_shape[i+2]}")
...
class _InverseSolveMixin():
    ...
    def solve(self, 
                ...
                return_in_fourier=False) -> torch.Tensor:
            if mesh is not None:
                if not isinstance(mesh,FourierMesh):
                    mesh=FourierMesh(mesh)
            if self._state_dict is None:
                if mesh is None or n_channel is None:
                    raise ValueError("No mesh has be registered, Mesh and n_channel should be given when calling solve")
                self.register_mesh(mesh,n_channel)
            else:
                if mesh is not None or n_channel is not None:
                    mesh = self._state_dict["f_mesh"] if mesh is None else mesh
                    n_channel = self._state_dict["n_channel"] if n_channel is None else n_channel
                    self.register_mesh(mesh,self._state_dict["n_channel"])
            if self._state_dict["invert_linear_coef"] is None:
                self._state_dict["invert_linear_coef"]=torch.where(self._state_dict["linear_coef"]==0,
                                                                   1.0,
...
class OperatorLike(_MutableMixIn):
    ...
    def is_linear(self):
        if self._state_dict is None:
            ...
            return self._state_dict["nonlinear_func"] is None and self._state_dict["linear_coef"] is not None

    def _build_linear_coefs(self,
                            linear_coefs:Optional[Sequence[LinearCoef]]) -> torch.Tensor:
        if len(linear_coefs)==0:
            linear_coefs=None
        else:
            linear_coefs=sum([coef*op(self._state_dict["f_mesh"],self._state_dict["n_channel"]) for coef,op in linear_coefs])
        self._state_dict["linear_coef"]=linear_coefs
        
    def _build_nonlinear_funcs(self,
                               nonlinear_funcs:Optional[Sequence[NonlinearFunc]]):  
        if len(nonlinear_funcs)==0:
            nonlinear_funcs_all=None
        else:
            self._state_dict["f_mesh"].set_default_freq_threshold(self._de_aliasing_rate)
                ...
                for coef,fun in nonlinear_funcs:
                    if fun._dealiasing_swtich:
                        if dealiased_u_fft is None:
                            dealiased_u_fft=u_fft*self._state_dict["f_mesh"].low_pass_filter(self._de_aliasing_rate)
                            dealiased_u=self._state_dict["f_mesh"].ifft(dealiased_u_fft).real
                        result+=coef*fun(dealiased_u_fft,
    ...
    def _build_operator(self):
        if self._state_dict["nonlinear_func"] is None:
            ...
            def operator(u_fft):
                return self._state_dict["linear_coef"]*u_fft+self._state_dict["nonlinear_func"](u_fft)
        self._state_dict["operator"]=operator       

    def _build_integrator(self,
                    dt:float,
                    **additional_solver_config):
        if self._etdrk_integrator=="auto":
            if self.is_linear:
                solver=ETDRKIntegrator.ETDRK0
            else:
                solver=ETDRKIntegrator.ETDRK4
        else:
            solver=self._etdrk_integrator
        if not solver.value["nonlinear"]:
            if not self.is_linear:
                raise ValueError(f"The solver {solver.name} does not support nonlinear term")
            self._state_dict["solver"] = solver.value["integrator"](dt,self._state_dict["linear_coef"],**additional_solver_config)
        else:
            if self._state_dict["linear_coef"] is None:
                linear_coef=torch.tensor([0.0],dtype=self._state_dict["f_mesh"].dtype,device=self._state_dict["f_mesh"].device)
            else:
                linear_coef=self._state_dict["linear_coef"]
            self._state_dict["solver"] = solver.value["integrator"](dt,linear_coef,self._state_dict["nonlinear_func"],**additional_solver_config) 
     
    def _pre_check(self,
                   u:Optional[torch.Tensor]=None,
                   u_fft:Optional[torch.Tensor]=None,
                   mesh: Union[Sequence[tuple[float, float, int]],MeshGrid,FourierMesh]=None,) -> Tuple[FourierMesh,int]:
        ...
        assert len(value_shape) == mesh.n_dim+2, f"the value shape {value_shape} is not compatible with mesh dim {mesh.n_dim}"
        for i in range(mesh.n_dim):
            assert value_shape[i+2] == mesh.mesh_info[i][2], f"Expect to have {mesh.mesh_info[i][2]} points in dim {i} but got {value_shape[i+2]}"
        assert value_device==mesh.device, "The device of mesh {} and the device of value {} are not the same".format(mesh.device,value_device)
        ...
        assert self._value_mesh_check_func(len(value_shape)-2,mesh.n_dim), "Value and mesh do not match the requirement"
    ...
    def register_mesh(self,
        ...
        self._state_dict={"f_mesh":f_mesh,
                     "n_channel":n_channel,
                     "linear_coef":None,
                     "nonlinear_func":None,
                     "operator":None,
    ...
    def set_etdrk_integrator(self,integrator:Union[Literal["auto"],ETDRKIntegrator]):
        self._etdrk_integrator=integrator
        self._state_dict["solver"] = None

    def build_integrator(self,dt:float,**additional_solver_config):
        if self._state_dict is None:
            raise ValueError("No mesh has be registered. Please register mesh before building integrator")
        self._build_integrator(dt,**additional_solver_config)
        return self._state_dict["solver"]
    ...
    def integrate(self, 
              ...
              **additional_solver_config) -> Optional[torch.Tensor]:
        if self._state_dict is None or mesh is not None:
            mesh,n_channel=self._pre_check(u=u_0,mesh=mesh)
            self.register_mesh(mesh,n_channel)
        else:
            self._pre_check(u=u_0,mesh=self._state_dict["f_mesh"])
        if self._state_dict["solver"] is None or additional_solver_config!={}:
            self._build_integrator(dt,**additional_solver_config)
        elif self._state_dict["solver"].dt!=dt:
            self._build_integrator(dt,**additional_solver_config)
        ...
        for i in p_bar:
            u_fft=self._state_dict["solver"].step(u_fft)
    ...
    def __call__(self, u:Optional[torch.Tensor]=None,
                u_fft:Optional[torch.Tensor]=None,
                mesh: Optional[Union[Sequence[tuple[float, float, int]],MeshGrid,FourierMesh]]=None,
                return_in_fourier=False) -> torch.Tensor:
            if self._state_dict is None or mesh is not None:
    ...
    def to(self,device=None,dtype=None):
        if self._state_dict is not None:
            self._state_dict["f_mesh"]=self._state_dict["f_mesh"].to(device=device,dtype=dtype)
...
class Operator(OperatorLike,_DeAliasMixin):
    ...
    def __add__(self, other):
        if isinstance(other,OperatorLike):
            return Operator(self.operator_generators+other.operator_generators,self.coefs+other.coefs)
        elif isinstance(other,Tensor):
            return Operator(self.operator_generators+[lambda f_mesh, n_channel: _ExplicitSourceCore(other)],
                            self.coefs+[1])
        else:
            return NotImplemented
    ...
    def __mul__(self, other):
        if isinstance(other,OperatorLike):
            ...
            return Operator(self.operator_generators,[coef*other for coef in self.coefs])
...
class LinearOperator(OperatorLike,_InverseSolveMixin):
    ...
    def __init__(self,
        ...
        super().__init__(
            operator_generators=[linear_coef_i if not isinstance(linear_coef_i,LinearCoef) else lambda f_mesh,n_channel: linear_coef_i for linear_coef_i in linear_coef],
            coefs=coefs
        )     
    ...
    def __add__(self, other):
        if isinstance(other,LinearOperator):
            return LinearOperator(self.operator_generators+other.operator_generators,self.coefs+other.coefs)
    ...
    def __mul__(self, other):
        if isinstance(other,OperatorLike):
            ...
            return LinearOperator(self.operator_generators,[coef*other for coef in self.coefs])
    
    def __neg__(self):
        return LinearOperator(self.operator_generators,[-1*coef for coef in self.coefs])
...
class NonlinearOperator(OperatorLike,_DeAliasMixin):
    def __init__(self,
            ...
            super().__init__(
                operator_generators=[nonlinear_func_i if not isinstance(nonlinear_func_i,NonlinearFunc) else lambda f_mesh,n_channel: nonlinear_func_i for nonlinear_func_i in nonlinear_func],
                coefs=coefs
            )
    ...
    def __add__(self, other):
        if isinstance(other,NonlinearOperator):
            return NonlinearOperator(self.operator_generators+other.operator_generators,self.coefs+other.coefs)
        elif isinstance(other,OperatorLike):
            return Operator(self.operator_generators+other.operator_generators,self.coefs+other.coefs)
        elif isinstance(other,Tensor):
            return Operator(self.operator_generators+[lambda f_mesh, n_channel: _ExplicitSourceCore(other)],
    ...
    def __mul__(self, other):
        if isinstance(other,OperatorLike):
            ...
            return NonlinearOperator(self.operator_generators,[coef*other for coef in self.coefs])        

    def __neg__(self):
        return NonlinearOperator(self.operator_generators,[-1*coef for coef in self.coefs])
