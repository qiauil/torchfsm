...
def _repeat_list(ori_list, n):
    re = []
    for n_i in ori_list:
        re.extend([n_i] * n)
    return re

def _prepare_error_traj_slice_data(
    ...
    slice_trajs = traj_slices(traj, slice_control)
    slice_error = traj_slices(error, slice_control)
    n_slice= len(slice_trajs)
    ...
    label_xs = _repeat_list(label_xs, 2)
    label_ys = _repeat_list(label_ys, 2)
    ticks_xs = _repeat_list(ticks_xs, 2)
    ticks_ys = _repeat_list(ticks_ys, 2)
    batch_names = []
    for slice_name in slice_names:
        batch_names.append(
            [f"{name_traj_1}, {slice_name}", f"{name_traj_2}, {slice_name}"]
        )
        batch_names.append([f"{error_name}, {slice_name}"])
    cmap = cmap * n_slice
    use_sym_colormap = use_sym_colormap * n_slice
    alpha_func = alpha_func * n_slice
    c_bar_labels = c_bar_labels * n_slice
    vmin = vmin * n_slice
    vmax = vmax * n_slice
    fields = []
    for i in range(n_slice):
        for field in [slice_trajs, slice_error]:
...
def compare_error_traj_slice(
    ...
    **kwargs,
    ...
    Returns:
        Optional[FuncAnimation]: The animation object if `animation` is True, otherwise None.
    """
    (
        fields,
        batch_names,
        ...
        ticks_xs,
        ticks_ys,
    ) = _prepare_error_traj_slice_data(
        traj_1=traj_1,
        traj_2=traj_2,
        error_func=error_func,
        slice_control=slice_control,
...
def compare_error_field_slice(
    ...
    **kwargs,
    ...
    return compare_error_traj_slice(
        ...
        label_x=label_x,
        label_y=label_y,
        label_z=label_z,  # Not used in field comparison
        ticks_x=ticks_x,
        ticks_y=ticks_y,
        ticks_z=ticks_z,  # Not used in field comparison
        show_ticks=show_ticks,
